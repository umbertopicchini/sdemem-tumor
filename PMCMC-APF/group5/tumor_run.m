...
problem = 'tumor';
group = 5; % select the treatment group to fit 
...
% remove dying mouse from group 1
if group==1
    logsubdata = logsubdata(logsubdata(:,3)~=14,:);
end
...
%startstate = 100; % [mm^3] the assumed volume initial state
...
step_rw = [0.1 0.1 0.1 0.1];%initial values for the standard deviations of adaptive Gaussian random walk 
...
numAPFsim = 5;      % number of particles required to compute the means in the first stage of the auxiliary particle filter

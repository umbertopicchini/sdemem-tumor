function [THETAmatrix,simsummaries] = synlike_sdmem_tumor_stopCoVadapt(problem,group,data,startstate,starttime,bigtheta,parmask,parbase,R_mcmc,step_rw,covariance_refresh,stopCoVadapt,numsim)

% A Metropolis-Hastings algorithm for BSL inference with an adaptive
% proposal kernel.
%
% Input:
...
%      - stopCoVadapt: stop adapting the covariance of the MCMC proposal after stopCoVadapt iterations.
%      - numsim: number of simulated artificial datasets used when computing the BSL approximation
% Output:
%      - THETAmatrix: a R_mcmc x sum(parmask) matrix of posterior draws, one column for each parameter to be inferred
%      - simsummaries: a vector of simulated summary statistic obtained at the last MCMC iteration
...
% matrix of parameters draws + some "distance measure" out of curiosity
MCMC(1,:) = theta_old;
...
[loglik_old,simsummaries] = tumor_synlike(startstate,starttime,bigtheta_old,data,numsim);
...
loglik = tumor_synlike(startstate,starttime,bigtheta,data,numsim);
...
for mcmc_iter = 3:R_mcmc
    ...
    %::::::::  Haario et al. (2001) "An adaptive Metropolis algorithm", Bernoulli Volume 7, 223-242.
       ...
       if (mcmc_iter < length_CoVupdate)
          ...
          theta = mvnrnd(theta_old,cov_current);
           ...
           if (mcmc_iter == lastCovupdate+length_CoVupdate) && mcmc_iter < stopCoVadapt
               ...
               covupdate = cov_update(MCMC(lastCovupdate+1:mcmc_iter-1,1:end),sum_old,length_CoVupdate-1,cov_old);
               sum_old = sum(MCMC(lastCovupdate+1:mcmc_iter-1,1:end));
               cov_old = cov(MCMC(lastCovupdate+1:mcmc_iter-1,1:end));
           ...
           elseif (mcmc_iter == lastCovupdate+length_CoVupdate) 
              ...
              % hence we use the same one obtained at last update
               ...
               MCMC_temp = MCMC(1:mcmc_iter-1,:);
               save('THETAmatrix_temp','MCMC_temp');
               lastCovupdate = mcmc_iter; % we are npt really updating anything but this is useful to print results on screen
           else
              % Here there is no "adaptation" for the covariance matrix,
              % hence we use the same one obtained at last update
                theta = mvnrnd(theta_old,cov_last);
    ...
    bigtheta = param_unmask(theta,parmask,parbase);
    [loglik,simsummaries] = tumor_synlike(startstate,starttime,bigtheta,data,numsim);
...
save('simsummaries','simsummaries')

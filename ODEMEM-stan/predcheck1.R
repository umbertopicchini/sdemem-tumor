# runs posterior predictive chacks for the fitted data from group 1
#
...
fit1 <- read.table('newfit1.txt', header=TRUE)
# BRUTAL HARDCODING! THIS IS THE FIRST MEASURED LOVOLUME FOR EACH SUBJECT
mylogv0 <- c(5.5033, 4.6032, 5.8808, 5.6781, 6.2943)
...
sumstat.inter <- function(y, n.obs){
  ...
  N<-sum(mydat1$n.obs) # total number of observations
  times_equal_last <- mydat1$frame$time==mydat1$frame$time[N]
...
inter.obs1 <- sumstat.inter(y=mydat1$frame$logvol, n.obs=mydat1$n.obs)
...
# Observed data summaries
intra.obs1 <- sumstat.intra(y=mydat1$frame$logvol, id=mydat1$frame$id, time=mydat1$frame$time)
...
sim.pred1 <- sim.pred.check(n.rep=10000, parms=fit1, id=mydat1$frame$id, time=mydat1$frame$time, initv0=mylogv0, my.seed=345)
# To be compared with
obs.pred1 <- c(inter.obs1, intra.obs1)

write.table(sim.pred1, 'newpostpred1.txt', row.names=FALSE)
